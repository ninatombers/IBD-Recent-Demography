...
rec_path <- "/Users/brigittetombers/Desktop/Project/5.Linkage-map/TRUFFLE/"
hypo_map1_cm <- "MAP1cm.txt"
hypo_map1_bp <- "MAP1bp.txt"
hypo_map2_cm <- "MAP2cm.txt"
hypo_map2_bp <- "MAP2bp.txt"
#segment_file <- read_tsv("/Users/brigittetombers/Desktop/Project/Linkage-map/truffle_2.5k_1k_nofilt.segments.tsv")
#summary_file <- read_tsv("truffle_2.5k_1k_nofilt.ibd.tsv")
#segment_file <- read_tsv("../Isolation_by_distance/truffle_1k_0.5k_nofilt.segments.tsv")
#summary_file <- read_tsv("../Isolation_by_distance/truffle_1k_0.5k_nofilt.ibd.tsv")
#segment_file <- as.character("/Users/brigittetombers/Desktop/Project/Linkage-map/truffle_2.5k_1k_nofilt.segments.tsv")
#summary_file <- as.character("/Users/brigittetombers/Desktop/Project/Linkage-map/truffle_2.5k_1k_nofilt.ibd.tsv")
segment_file <- as.character("/Users/brigittetombers/Desktop/Project/5.Linkage-map/TRUFFLE/truffle_1k_0.5k_95.segments.tsv")
summary_file <- as.character("/Users/brigittetombers/Desktop/Project/5.Linkage-map/TRUFFLE/truffle_1k_0.5k_95.ibd.tsv")
...
#segments_individual <- (segment_file) %>%
...
cM_treshold <- 0.2
segments_filterd <- converted_segments %>%
  filter(length_cM_m1 > cM_treshold & length_cM_m2 > cM_treshold)

ggplot(segments_filterd, aes(x = length_cM_m1 , y = length_bp)) +
  geom_point() +
  geom_smooth(method = 'lm', se=FALSE) +
  ggtitle("map1-bp") +
  theme_minimal()

ggplot(segments_filterd, aes(x = length_cM_m2 , y = length_bp)) +
  geom_point() +
  geom_smooth(method = 'lm', se=FALSE) +
  ggtitle("map2-bp") +
  theme_minimal()

model <- lm(length_cM_m1 ~ length_bp, segments_filterd)
s <- summary(model)
s$r.squared 
...
write_tsv(x = segments_filterd, file = "truffle_segments_filt.tsv")
